...
def _request(method, path, body=None):
    ...
    try:
        ...
        raise RuntimeError(f"Clash API {method} {path} не сработал: {e}")
    ...
    if node:
        ...
        for n in all_nodes:
            if get_node_country(n) == region:
                print(f"[Clash] исходная стабильная нода '{node[:30]}' исчезла, откат на ноду того же региона: {n[:40]}")
                return n
        print(f"[Clash] исходная стабильная нода '{node[:30]}' исчезла, нод того же региона нет, автопоиск")
    # 自动检测
...
def restore(node):
    ...
    all_nodes, _ = list_nodes()
    if node in all_nodes:
        print(f"[Clash] восстанавливаю ноду: → {node[:50]}")
        switch_node(node)
    else:
        # 原节点已消失（订阅更新），fallback 到同区域
        region = get_node_country(node)
        print(f"[Clash] исходная нода '{node[:30]}' исчезла, ищу замену в регионе {region}...")
        for n in all_nodes:
            if get_node_country(n) == region:
                print(f"[Clash] восстановление: → {n[:50]}")
                switch_node(n)
                return
        print(f"[Clash] не удалось восстановить, оставляю текущую ноду")
...
def switch_to_stable(stable_node=None):
    ...
    if target == current:
        return target
    print(f"[Clash] переключаюсь на стабильную ноду: {current[:40]} → {target[:40]}")
    switch_node(target)
    return target
...
def random_switch(exclude_stable=True):
    ...
    print(f"[Clash] случайное переключение: {current[:30]}... → {target[:30]}...")
    switch_node(target)
    return target
...
def switch_region(exclude_regions=None):
    ...
    print(f"[Clash] смена региона: {current_region}({current[:20]}...) → {target_region}({target[:20]}...)")
    switch_node(target)
...
if __name__ == "__main__":
    ...
    if args.snapshot:
        ...
        h = health()
        print(f"нода: {h.get('current_node','?')}")
        print(f"  IP: {h.get('current_ip','?')}")
        print(f"регион: {h.get('region','?')}")
