...
def wait_for_callback(verifier: str, timeout: int = 180) -> dict:
    ...
    print(f"Browser will open the authorization page; sign in and click \"Allow\" within {timeout}s", file=sys.stderr)
...
def main():
    ...
    if "error" in got:
        print(f"ERROR: authorization failed — {got['error']}", file=sys.stderr)
        sys.exit(1)
    if "code" not in got:
    ...
    if "refresh_token" not in token:
        print(f"ERROR: no refresh_token in response: {json.dumps(token)[:500]}", file=sys.stderr)
        sys.exit(1)

    account = decode_id_token_account(token.get("id_token", ""), account_hint) or "unknown"
    print(f"authorized OK, account: {account}", file=sys.stderr)
    ...
    print(f"saved to {TOKENS_FILE}", file=sys.stderr)
    ...
    try:
        ...
        conn.logout()
        print(f"IMAP verification OK: INBOX {total} msgs, Junk folder {'present' if junk_ok else 'MISSING'}", file=sys.stderr)
    except Exception as e:
        print(f"IMAP verification failed: {e}", file=sys.stderr)
