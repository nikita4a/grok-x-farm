    ...
    async with async_playwright() as p:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                print(f"  [TSolver] attempt {attempt}/{max_attempts}: loading {site_url[:60]}...")
                ...
                if trigger_selector:
                    try:
                        if trigger_selector == "__xai_email__":
                            ...
                            print(f"  [TSolver] manually rendering xAI Turnstile...")
                            # 1) гарантируем загрузку скрипта Turnstile (страница может его не подгрузить)
                            await page.evaluate('''() => new Promise((resolve, reject) => {
                                if (window.turnstile) return resolve(true);
                                const t = setTimeout(() => reject(new Error('turnstile script load timeout')), 20000);
                                const s = document.createElement('script');
                                s.src = 'https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit&onload=__grok_ts_onload';
                                window.__grok_ts_onload = () => { clearTimeout(t); resolve(true); };
                                s.onerror = () => { clearTimeout(t); reject(new Error('turnstile script load failed')); };
                                document.head.appendChild(s);
                            })''')
                            # 2) рендерим виджет (только если токена ещё нет)
                            await page.evaluate(f'''() => {{
                                if (document.querySelector("[name=cf-turnstile-response]")?.value) return;
                            ...
                            if await btn.count() > 0:
                                print(f"  [TSolver] clicking trigger: {trigger_selector}")
                                await btn.click()
                        await asyncio.sleep(3)
                    except Exception as e:
                        print(f"  [TSolver] trigger failed: {e}")

                # 等待 Turnstile 解决
                print(f"  [TSolver] waiting for Turnstile (max {captcha_timeout}s)...")
                poll_interval = 0.5
                ...
                print(f"  [TSolver] attempt {attempt} timed out")
                last_error = f"Timeout after {captcha_timeout}s"

            except Exception as e:
                last_error = str(e)[:200]
                print(f"  [TSolver] attempt {attempt} error: {last_error}")
