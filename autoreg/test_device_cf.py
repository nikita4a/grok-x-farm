...
print("\n=== Step 2: browser authorization ===", flush=True)
opts = FirefoxOptions()
...
try:
    ...
    if "Attention Required" in page.title:
        print("CF challenge detected, handling...", flush=True)
        ok = page.handle_cloudflare_challenge(timeout=60)
        print(f"CF handling result: {ok}", flush=True)
        time.sleep(3)
    ...
    if "device/consent" in page.url and "Attention Required" not in page.title:
        print("device consent page, clicking Allow...", flush=True)
        try:
            btn = page.ele("@text()=Allow", timeout=3)
            if btn:
                btn.click()
                print("clicked Allow", flush=True)
                time.sleep(3)
...
# Step 3: 轮询 token
print("\n=== Step 3: polling token ===", flush=True)
for i in range(36):
