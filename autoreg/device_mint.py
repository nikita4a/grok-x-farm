...
def sso_to_device(sso_token, email=""):
    ...
    try:
        ...
        if s != 200 or not isinstance(p, dict) or "device_code" not in p:
            print(f"  [DEVICE] device/code failed HTTP {s}: {str(p)[:150]}")
            return None
        dc = p["device_code"]
        vuc = p.get("verification_uri_complete")
        ok = asyncio.run(_browser_authorize(vuc, sso_token))
        if not ok:
            print("  [DEVICE] authorization not confirmed, continuing to poll")
        ...
        while time.time() < deadline:
            ...
            if err in ("access_denied", "expired_token"):
                print(f"  [DEVICE] token failed: {err}: {t.get('error_description', '')[:80]}")
                return None
            if err == "slow_down":
                interval += 5
        print("  [DEVICE] poll timed out")
        return None
    except Exception as e:
        print(f"  [DEVICE] exception: {str(e)[:120]}")
        return None
...
if __name__ == "__main__":
    ...
    print(f"test mint: {email}")
    result = sso_to_device(sso_map.get(email, ""), email)
    if result:
        print(f"OK: AT={result['access_token'][:30]}... RT={result['refresh_token'][:20]}...")
    else:
        print("FAILED")
